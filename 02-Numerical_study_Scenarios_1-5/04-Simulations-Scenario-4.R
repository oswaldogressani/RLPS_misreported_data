...
set.seed(1455)             # Seed for reproducibility
...
if(s==5){prob_init <- rep(0.15, (dim_eta - 2))}
if(s==23){prob_init <- rep(0.40, (dim_eta - 2))}
if(s==34){prob_init <- rep(0.15, (dim_eta - 2))}
if(s==44){prob_init <- rep(0.15, (dim_eta - 2))}
if(s==79){prob_init <- rep(0.55, (dim_eta - 2))}
if(s==89){prob_init <- rep(0.15, (dim_eta - 2))}
if(s==92){prob_init <- rep(0.45, (dim_eta - 2))}
if(s==99){prob_init <- rep(0.15, (dim_eta - 2))}
if(s==117){prob_init <- rep(0.15, (dim_eta - 2))}
if(s==121){prob_init <- rep(0.15, (dim_eta - 2))}
if(s==124){prob_init <- rep(0.55, (dim_eta - 2))}
if(s==132){prob_init <- rep(0.15, (dim_eta - 2))}
if(s==136){prob_init <- rep(0.15, (dim_eta - 2))}
if(s==150){prob_init <- rep(0.55, (dim_eta - 2))}
if(s==154){prob_init <- rep(0.15, (dim_eta - 2))}
if(s==174){prob_init <- rep(0.45, (dim_eta - 2))}
if(s==181){prob_init <- rep(0.55, (dim_eta - 2))}
if(s==199){prob_init <- rep(0.15, (dim_eta - 2))}
if(s==232){prob_init <- rep(0.15, (dim_eta - 2))}
if(s==266){prob_init <- rep(0.15, (dim_eta - 2))}
if(s==328){prob_init <- rep(0.05, (dim_eta - 2))}
if(s==342){prob_init <- rep(0.15, (dim_eta - 2))}
if(s==354){prob_init <- rep(0.15, (dim_eta - 2))}
if(s==408){prob_init <- rep(0.15, (dim_eta - 2))}
if(s==466){prob_init <- rep(0.15, (dim_eta - 2))}
if(s==483){prob_init <- rep(0.15, (dim_eta - 2))}
...
# Save plot
pdf(file="Figure3_R_Scenario_4_oneday.pdf", width = 14, height = 5)
...
legend("topright",c("Target","Estim. LPS","Median LPS","Median EpiEstim"),
       ...
       lty=c(1,1,2,4),lwd=c(1,1,2,2),bty="n",cex=0.8)
axis(side = 2, seq(0, 8, by = 1))


# Mean absolute percentage error for Rt
...
barplot(matrix(as.vector(t(MAPE_mat)),nr=3),beside=T,
        ...
        names.arg = seq(8,Ttime),main="MAE of R", cex.main=0.8, yaxt="none")
legend("topright",col=c("deepskyblue1","deepskyblue4","darkolivegreen2"),
       c("EpiEstim","EpiEstimNM","LPS"),bty="n",pch=15)
axis(side=2,seq(0,round(max(MAPE_mat)+0.4,2),by=0.2))
...
write.xlsx(round(perf_metrics,3),"Scenario4-Summary.xls")


print(paste("Average time for estimating the model:",round(toc_total/S,2),
            "seconds."))
