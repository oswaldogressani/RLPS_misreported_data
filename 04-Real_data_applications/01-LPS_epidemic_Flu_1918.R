...
K <- 20                  # Number of (cubic) B-splines in basis
...
delay_struct <- "oneday" # Assumed delay structure
...
p <- c(Flu1918$si_distr[2], Flu1918$si_distr[3], # Generation interval 
      (1 - (Flu1918$si_distr[2] + Flu1918$si_distr[3])))
kmax <- length(p)                       

Ot <- Flu1918$incidence
Ttime <- length(Ot)
tl <- 0
tr <- Ttime
dim_eta <- 9
...
# dlogpetaR(c(log(NR$eta_optim[1]),
...
Rwateu <- wallinga_teunis(Mtestim, 
                          method="non_parametric_si",
                          config=list(t_start = seq(2, 86), t_end = seq(8, 92),
                                      si_distr = c(0,p),
                                      n_sim = 100))

# Save plots
pdf(file="Figure4_Flu1918_Incidence.pdf", width = 12, height = 6)
plot(as.incidence(Flu1918$incidence))
dev.off()


pdf(file="Figure4_Flu1918_Rt2.pdf", width = 12, height = 6)

plot(seq(8,Ttime),Rthat,col="black",type="l",xlab="Time",ylab="R",
     xlim=c(0,Ttime), ylim=c(0,2.7),
     main="Estimated R", cex.main=0.92) #Estimated R with LPS
...
legend("topright",c("LPS","EpiEstim","Wallinga-Teunis"),
...
LPS_results <- matrix(0, nrow = dim_eta-1, ncol = 3)
rownames(LPS_results) <- c("rho","dMoTu","dTuWe","dWeTh","dThFr","dFrSa",
                           "dSaSu","dSuMo")
colnames(LPS_results) <- c("Point estimate","CI95.low","CI95.up")
LPS_results[,1] <- c(rho_hat, delta_hat)
LPS_results[, 2] <- CIrhodelta[, 1]
LPS_results[, 3] <- CIrhodelta[, 2]
