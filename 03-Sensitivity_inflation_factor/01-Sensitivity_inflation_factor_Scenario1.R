...
#             Sensitivity to inflation factor for Scenario 1                   #
...
rho_tilde <- c(0.05,0.10,0.15,0.20,0.25,0.30,0.35)
...
# Save plot
pdf(file="Figure6_R_Scenario_1_sensitivity.pdf", width = 13, height = 6)

plot(tcont, R_smoothmed_LPS[1,], type="l", col="chartreuse 3", ylab="R",
     xlab="Time", ylim =c(0,11), lwd=2,
     main=paste0("Estimated R (Scenario 1)"), 
...
lines(tcont, sapply(tcont,Rsmooth), type="l",col="black",lwd=2, lty=2)
...
legend(x=0, y=11,
       text.width = rep(2.5,8),
      c(expression(paste(rho,"= 0.05")),
        expression(paste(rho,"= 0.10")), 
        expression(paste(rho,"= 0.15")),
        expression(paste(rho,"= 0.20")),
        expression(paste(rho,"= 0.25")),
        expression(paste(rho,"= 0.30")),
        expression(paste(rho,"= 0.35")),
        "Target"
       ),
...
rownames(perf_metrics) <- rep("Scenario 1", nmult)
