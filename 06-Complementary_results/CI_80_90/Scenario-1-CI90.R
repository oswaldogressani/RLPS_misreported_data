...
#               Simulation results for Scenario 1 (90% CI)                     #
...
alpha <- 10/100            # Level for credible intervals
...
prob_init <- pdelay +  runif(7, min = -0.15, max = 0.15)
rho_init <- rho + runif(1, min = -0.1, max = 0.1)
lambda_init <- 150000
...
colnames(perf_metrics) <- c("True","Mean","Bias","ESE","RMSE","CI90.low",
                            "CI90.up",
...
# Save plot
pdf(file="Scenario_1_CI90.pdf", width = 20, height = 6)
...
plot(tdom, sapply(tdom, Rsmooth), type="l", col="black", ylab="R",
     ...
     main="Estimated R (Scenario 1)", 
     cex.main=0.9, yaxt="none")
grid(10,10)
...
barplot(matrix(as.vector(t(MAPE_mat)),nr=3),beside=T,
        ...
        names.arg = seq(8,Ttime),main="Mean absolute error of R", 
        cex.main=0.9, yaxt="none", xlab = "Time")
...
write.xlsx(round(perf_metrics,3),"Scenario1-CI90.xls")

# Save workspace
save.image("Scenario1-CI90.RData")
