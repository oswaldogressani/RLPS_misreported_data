...
#                 Effect of time-dependent rho (linear)                        #
...
rho <- NA                  # True reporting probability
...
# Function describing how rho evolves over time
rho_fun <- function(t) {
  if (t < 8) {
    res <- 0.2
  } else if (t >= 8 && t < 20) {
...
if(s==25){rho_init <- 0.5; prob_init <- rep(0.30, (dim_eta - 2))}
if(s==66){rho_init <- 0.5; prob_init <- rep(0.30, (dim_eta - 2))}
if(s==82){rho_init <- 0.48; prob_init <- rep(0.35, (dim_eta - 2))}
...
# Save plot
pdf(file="RhoStepFun.pdf", width = 20, height = 6)

par(mfrow=c(1,2))

## Plot evolution of rho over time
rho_fun <- function(t) {
  if (t < 8) {
    res <- 0.2
  } else if (t >= 8 && t < 20) {
...
plot(tdom, sapply(tdom, Rsmooth), type="l", col="black", ylab="R",
     xlab="Time", ylim =c(0,12),
     main="Estimated R (Scenario 4)", 
     cex.main=1.1, cex.axis = 1.3, cex.lab = 1.3, yaxt="none")
grid(10,10)
lines(tdom,apply(Rthat,2,"median"),col="red",lty=2,lwd=2)
lines(tdom, sapply(tdom,Rsmooth), type="l",col="black",lwd=2)
abline(h=1, lty=3,col="black")
legend("top",c("Target","Estim. LPS","Median LPS"),
       col=c("black","gray70","red"),
       lty=c(1,1,2,4),lwd=c(1,1,2,2),bty="n",cex=1.1)

dev.off()

# Save workspace
save.image("RhoStepFun.RData")
